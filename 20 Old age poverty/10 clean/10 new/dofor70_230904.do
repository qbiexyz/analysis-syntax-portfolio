...
******************************************************************************
est clear
use "10 data\家庭收支調查\70\inc70.dta",clear
*/
...
*與戶長關係
recode b2_ (1/2 = 0 "本人/配偶")(8 = 1 "子女")(3 4 = 2 "父母輩") ///
...
///先設定若干條件 ///
gen snd1 = 0
replace snd1 = 1 if  b2_ == 8
bys id: egen snd = sum(snd1)
la var snd "與戶長關係為子女數"

gen elsemember0 = 0
replace elsemember0 = 1 if b2_ == 3 | inrange(b2_, 5, 7) | inrange(b2_, 9, 12)
bys id:egen elsemember= sum(elsemember0) 
la var elsemember "與戶長關係為其他人數"

gen parents0 = 0
replace parents0 = 1 if b2_ == 4
